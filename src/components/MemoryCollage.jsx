...
export function MemoryCollage({ open, onOpen, onClose }) {
  const { items, loading, error, assetUrl, refetch } = useMemoriesManifest();
  ...
  const itemsKey = useMemo(
    () => items.map((it) => `${it.file}:${it.kind}`).join("|"),
    [items],
  );
          ...
          <AnimatePresence>
            {open && (
              ...
              >
                ...
                {loading && (
                  <p className="relative z-10 font-display text-2xl text-white/80">
                    Loading memories…
                  </p>
                )}

                {!loading && error && (
                  <p className="relative z-10 max-w-md px-6 text-center font-sans text-white/85">
                    {error}
                  </p>
                )}
                ...
                {!loading && !error && urls.length === 0 && (
                  <div className="relative z-10 mx-6 max-w-md rounded-2xl border border-white/20 bg-white/10 px-6 py-10 text-center backdrop-blur-md">
                    ...
                    <p className="mt-3 font-sans text-sm text-white/65">
                      Add files to{" "}
                      <span className="text-blush">public/memories/</span> and
                      run <span className="text-blush">npm run dev</span> or{" "}
                      <span className="text-blush">
                        npm run memories:manifest
                      </span>
                      .
                    </p>
                  </div>
                          ...
                          {visibleItems.map((item, i) => (
                            <motion.div
                              ...
                              whileHover={{
                                scale: 1.03,
                                boxShadow:
                                  "0 0 0 2px rgba(255,192,203,0.5), 0 24px 48px rgba(0,0,0,0.35)",
                                zIndex: 5,
                              }}
                            >
                              {item.kind === "image" ? (
